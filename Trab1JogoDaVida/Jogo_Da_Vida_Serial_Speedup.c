...
long int tempo_newgen, tempo_count_final;
...
void countFinalCells(int *grid, int generation){ //Conta o total de celulas vivas no fim da i geração
    ...
    for(i=0; i < DIMENTION; i++){
        for (j = 0; j < DIMENTION; j++)
            grid[i * DIMENTION + j]==1? n++ : n;
    }

   gettimeofday(&end, NULL);

  tempo_count_final = ((end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
     printf("Geracao %d: %d\n", generation, n);
}
...
void newGen(int *gen, int* auxGen){
  ...
  tempo_newgen = ((end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec));
...
int main(){
    ...
    float tempo_pararelo = ((tempo_newgen + tempo_count_final)/1000);
   ...
   float speedup = (1/(1-(tempo_pararelo/elap)));
    printf("Tempo pararelo: %.2f\n", tempo_pararelo);
    printf("Tempo total: %.4f secs\n", elap);
    printf("Speedup_limite: %.2f", speedup);
