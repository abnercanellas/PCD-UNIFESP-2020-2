...
void countFinalCells(int *grid, int generation){ //Conta o total de celulas vivas no fim da i geração
    // struct timeval be, ed;
    // gettimeofday(&be,0);

    int i, j, thr, n[THREADS];

    for(int k=0; k < THREADS; k++) n[k] = 0;

    #pragma omp parallel shared(grid, n) private(i,j, thr) num_threads(THREADS)
    {  
        #pragma omp for
        for(i=0; i < DIMENTION; i++){
            for (j = 0; j < DIMENTION; j++) {
    ...
    // gettimeofday(&ed,0);
    // long sec = ed.tv_sec - be.tv_sec;
    // long mic = ed.tv_usec - be.tv_usec;
    // double elap = sec + mic*1e-6;
    // printf("Tempo total: %.4f sec", elap); 
    
    // Serial/1Thread func=0.0096s real=2m28,036s
    // 2Threads func=0.0061s real=1m31,944s
    // 4Threads func=0.0042s real=1m7,671s
    // 8Threads func=0.0039s real=1m4,627s
